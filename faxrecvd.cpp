...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    std::string faxnumid = "";
    db.queryDB(db.prepareStatement("AddressBookFax", "abook_id", "faxnumber = " +  + "'" + fax_info.CallID1 + "'",,,SELECT | FROM | WHERE));
    if(db.hasResults())//Find out if the fax number is registered in the address book
    {
        db.getResult(faxnumid);
        db.queryDB(db.prepareStatement("AddressBook", "Company", "abook_id = " + intToStr(faxnumid),,,SELECT|FROM|WHERE));
    ...
    else//Create a new entry for this fax with the information available
    {
        ...
        db.query(db.prepareStatement("AddressBook", "Company", fax_info.CallID2,,,INSERT|INTO));
        //Select the autogenerated id for CallID2 in AddressBook after insertion
        db.query(db.prepareStatement("AddressBook", "abook_id", "Company = " +  + "'" + fax_info.CallID2 + "'",,,SELECT|FROM|WHERE));
        db.getResult(faxnumid);
        db.query(db.prepareStatement("AddressBookFax", "abook_id,faxnumber,email,description,to_person,to_location," +
    ...
    db.queryDB(db.prepareStatement("didroute", "didr_id", "routecode = " +  + "'" + fax_info.CallID3 + "'",,,SELECT | FROM | WHERE));
    ...
    else//Create a new entry for this fax with the information available
    {
        ...
        db.query(db.prepareStatement("didroute", "routecode,alias,contact,printer,faxcatid",
    ...
    db.queryDB((db.prepareStatement("modems", "faxcatid", "device = " + args.modemdev,,,SELECT|FROM|WHERE)));
    if(db.hasResults())
    {
    ...
    {
        h2fax::logMsg("Oops, the ship sunk! This is why we can't have nice things... The program was not able" +
    ...
    db.queryDB(db.prepareStatement("UserAccount", "email", "modemdevs = " + args.modemdev,,,SELECT|FROM|WHERE));
    if(db.hasResults())
    {
        std::string tmp;
        recipient += "\"";
